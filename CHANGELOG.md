...
If you only use the shell implementation, you just need `com.github.joyoe.libsu:core`. `com.github.joyoe.libsu:io` and `com.github.joyoe.libsu:busybox` are optional: include the former to use the I/O wrapper classes, and the latter to bundle the prebuilt busybox binaries with your app. The old `com.github.joyoe:libsu` can still be used, but all 3 components will be pulled in.
...
### Breaking Changes
- Remove the class `BusyBox`. To install the prebuilt busybox, add `com.github.joyoe.libsu:busybox` as a dependency, and register `BusyBoxInstaller` as an initializer (`Shell.Config.addInitializers(BusyBoxInstaller.class);`)
